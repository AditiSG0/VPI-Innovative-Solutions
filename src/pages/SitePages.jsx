...
function ProcessSteps() {
  const steps = [
    ["01", "Share your requirement", "Send the drawing or model with material and quantity details.", "DRAWING / CAD", productImages.collet],
    ["02", "Engineering review", "Our team reviews the manufacturing requirements and production approach.", "ENGINEERING / REVIEW", machineImages[1]],
    ["03", "Precision machining", "The component is produced using VPI’s CNC machining capabilities.", "CNC / MACHINING", productImages.components],
    ["04", "Inspection & delivery", "Finished parts are inspected and prepared for delivery.", "QUALITY / DELIVERY", productImages.lineup],
  ];
    ...
    <section className="management-profiles page-pad" data-theme="dark">
      <Reveal className="management-profile management-profile-ceo" onMouseMove={spotlightMove}>
        ...
        <div className="management-profile-copy">
          ...
          <div className="management-role">( Managing Directer &amp; CEO )</div>
...
export function MediaPage() {
  const posts = [
    ["01", "Next-Generation CNC Machining Technologies", "April 28, 2025", "Explore how VPI is revolutionizing precision manufacturing with advanced collet chuck systems that enhance productivity while minimizing downtime.", machineImages[1]],
    ["02", "VPI Expands International Partnership Network", "May 2, 2025", "VPI Innovative Solutions announces strategic partnerships with leading European manufacturers to enhance global supply chain capabilities.", productImages.lineup],
    ["03", "Introducing the VG-20 CNC Collet Chuck Series", "May 5, 2025", "Our newest precision-engineered solution delivers exceptional concentricity and stability for high-speed machining applications.", "/vpi/vg20-angle-2.jpg"],
  ];
  return <div><PageHero number="01" eyebrow="MEDIA" title="VPI’s Innovative" accent="Media" lead="Blogs and Updates" image={productImages.lineup} /><PageIntro kicker="Blogs and Updates" title={<>Stay updated with the latest insights, trends, and <em>innovations</em> in the manufacturing industry.</>}><p className="page-lead">Our Blogs and Updates section brings you detailed articles, technical write-ups, and news on the cutting-edge projects we are working on, industry advancements, and much more.</p></PageIntro><section className="media-grid page-pad" data-theme="dark">{posts.map(([number,title,date,excerpt,image]) => <Reveal key={title}><article className="media-card"><div className="media-card-image"><img src={image} alt={title} loading="lazy" /></div><div className="media-card-body"><span className="service-number">{number}</span><h3>{title}</h3><div className="media-meta"><span>{date}</span><span>VPI / LinkedIn</span></div><p>{excerpt}</p><a href="https://in.linkedin.com/company/vpi-innovative-solutions-mysore" target="_blank" rel="noreferrer" className="text-link">READ ON LINKEDIN <ArrowUpRight size={14}/></a></div></article></Reveal>)}</section></div>;
}

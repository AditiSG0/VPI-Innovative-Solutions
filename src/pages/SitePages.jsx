...
const industries = [
  ["Automotive", "/industries/automotive", "Precision components that drive the future of mobility with unmatched durability and performance."],
...
const featuredIndustries = [
  ["Robotics", "/industries/robotics", "/industry/robotics.png", "Lightweight, high-strength components enabling the next generation of automation."],
  ["Medical", "/industries/medical", "/industry/medical.png", "Biocompatible, precision-engineered solutions that meet the strictest healthcare standards."],
  ["Electronics", "/industries/electronics", "/industry/electronics.png", "Micro-precision components for the devices powering our connected world."],
  ["Automotive", "/industries/automotive", "/industry/automotive.png", "Precision components that drive the future of mobility with unmatched durability and performance."],
  ["Aerospace", "/aerospace-industry", "/industry/aerospace.png", "Precision components for aviation and space applications."],
];
...
function ProcessSteps() {
  const steps = [
    ["01", "Custom Machining", "Tailored to exact design and tolerance requirements.", "CUSTOM MACHINING", machineImages[1]],
    ["02", "Precision Expertise", "Decades of experience in high-accuracy machining.", "PRECISION EXPERTISE", productImages.components],
    ["03", "Scalable Solutions", "From single parts to batch runs with ease.", "SCALABLE SOLUTIONS", machineImages[2]],
    ["04", "Quality Control", "Integrated CMMs, tool presetters, and more.", "QUALITY CONTROL", productImages.collet],
  ];
...
export function HomePage() {
  ...
  return <div data-testid="home-page">
    ...
    <PrecisionScene />
    <section className="home-overview page-pad" data-theme="steel"><Reveal className="overview-image"><img src={productImages.collet} alt="VPI CNC Collet Chuck" /></Reveal><Reveal className="overview-copy"><span className="eyebrow cyan">VPI PRECISION COMPONENTS</span><h2>VPI Precision <em>Components</em></h2><p className="body-copy">Discover our high-quality metal components and precision engineering solutions.</p><LinkButton to="/products">EXPLORE PRODUCTS</LinkButton></Reveal></section>
    ...
    <section className="featured-industries page-pad" data-theme="steel"><Reveal><div className="section-heading-row"><div><span className="eyebrow cyan">INDUSTRIES WE MAKE AN IMPACT</span><h2>Precision Engineering<br /><em>Reimagined</em></h2></div><Link to="/industries" className="text-link">VIEW ALL INDUSTRIES <ArrowUpRight size={15} /></Link></div></Reveal><div className="featured-industry-grid">{featuredIndustries.map(([name, path, image, copy], index) => <Reveal key={path} delay={index * 0.04}><Link to={path} className="featured-industry-card" onMouseMove={spotlightMove}><div className="featured-industry-media"><img src={image} alt={`${name} industry`} loading="lazy" /></div><div className="featured-industry-overlay"><span className="service-number">0{index + 1}</span><div><h3>{name}</h3><p>{copy}</p></div><ArrowUpRight size={17} /></div></Link></Reveal>)}</div></section>
    ...
    <section className="home-vision page-pad" data-theme="black"><Reveal><span className="eyebrow cyan">OUR VISION</span><h2>OUR <em>VISION</em></h2><p className="page-lead">{visionText[0]}<br />{visionText[1]}<br />{visionText[2]}<br />{visionText[3]}</p><LinkButton to="/vision">VIEW COMPANY VISION</LinkButton></Reveal></section>
    <VpiVisionFooter />
  </div>;
}
...
export function AerospacePage() { return <OfficialPage eyebrow="AEROSPACE INDUSTRY" title="Aerospace" accent="Industry" lead="Welcome to VPI Innovative Industries" kicker="Aerospace Summary" image="/industry/aerospace.png" paragraphs={["We specialize in manufacturing mission-critical components for advanced and specialized applications that demand the highest levels of accuracy, stability, and material performance.", "Our precision engineering capabilities support high-accuracy, high-performance components for aviation and space applications."]} bullets={["Precision components for aviation and space", "Multi-axis CNC machining", "High-performance materials", "Precision validation and inspection"]} extraTitle="Aerospace Industries" extraParagraphs={["We support demanding applications with high-performance materials, advanced machining processes, and disciplined quality control."]} />; }
export function DieMouldPage() { return <IndustryPage data={industryData["die-mould"]} />; }
export function EnergyPage() { return <IndustryPage data={industryData.energy} />; }
export function FoodPage() { return <IndustryPage data={industryData.food} />; }
...
export function CNCColletChucksPage() {
  const features = [
    "DIN 2363 series compatibility",
    "Ultra-precise concentricity",
    "Quick-change functionality",
    "Extended tool life",
    "25% more affordable than imports",
  ];
  const vg20Series = [
    "Tribute to founder's legacy",
    "Rivals international standards",
    "75% cost savings",
    "Seamless CNC integration",
    "Inventory optimization",
  ];
  return <div><PageHero number="01" eyebrow="CNC COLLET CHUCKS" title="Precision CNC Tooling" accent="Solutions" lead="Engineered for performance, built for durability." image={productImages.collet} /><PageIntro kicker="CNC Collet Chucks" title={<>High-precision <em>Collet Chucks</em></>}><p className="page-lead">High-precision collet chucks designed for maximum grip and minimal runout in CNC applications.</p><div className="detail-list">{features.map((item) => <div key={item}><CheckCircle2 size={16}/><span>{item}</span></div>)}</div><LinkButton to="/contact">GET QUOTE</LinkButton></PageIntro><PrecisionScene /><section className="page-pad" data-theme="steel"><span className="eyebrow cyan">VG-20 Series</span><h2>Our Flagship <em>Line</em></h2><p className="page-lead">Our flagship line of machine tool accessories combining international quality with local affordability.</p><div className="detail-list">{vg20Series.map((item) => <div key={item}><CheckCircle2 size={16}/><span>{item}</span></div>)}</div></section><RangeSection kicker="VG-20 Collet Chuck Full Specifications" title="VG-20" accent="Specifications" headers={vgHeaders} rows={vgRows} note="VG-20 CNC Collet Chuck" prefix="vg20" /><VpiVisionFooter /></div>;
}

export function RevolvingCentresPage() { return <div><PageHero number="01" eyebrow="PRODUCTS" title="Revolving" accent="Centers" lead="Premium revolving centers that provide exceptional support for long workpieces in turning operations." image={productImages.revolving} /><PageIntro kicker="Revolving Centers" title={<>Precision Support for<br /><em>Turning Operations</em></>}><div className="detail-list">{["High rotational accuracy", "Reduced vibration", "Extended bearing life", "Heavy-duty construction", "Compatible with all major brands"].map((item) => <div key={item}><CheckCircle2 size={16}/><span>{item}</span></div>)}</div><LinkButton to="/contact">GET QUOTE</LinkButton></PageIntro><VpiVisionFooter /></div>; }
export function QuickChangeColletsPage() { return <div><PageHero number="01" eyebrow="PRODUCTS" title="Quick-Change" accent="Collets" lead="High-precision collet chucks designed for maximum grip and minimal runout in CNC applications." image={productImages.collet} /><PageIntro kicker="CNC Collet Chucks" title={<>Quick-Change <em>Functionality</em></>}><p className="page-lead">High-precision collet chucks designed for maximum grip and minimal runout in CNC applications.</p><div className="detail-list">{["DIN 2363 series compatibility", "Ultra-precise concentricity", "Quick-change functionality", "Extended tool life", "25% more affordable than imports"].map((item) => <div key={item}><CheckCircle2 size={16}/><span>{item}</span></div>)}</div><LinkButton to="/contact">GET QUOTE</LinkButton></PageIntro><VpiVisionFooter /></div>; }

export function MediaPage() {
  const posts = [
    ["01", "Next-Generation CNC Machining Technologies", "April 28, 2025", "Explore how VPI is revolutionizing precision manufacturing with advanced collet chuck systems that enhance productivity while minimizing downtime.", productImages.revolving],
    ["02", "VPI Expands International Partnership Network", "May 2, 2025", "VPI Innovative Solutions announces strategic partnerships with leading European manufacturers to enhance global supply chain capabilities.", machineImages[0]],
...
export function ContactPage() { return <div><PageHero number="01" eyebrow="CONTACT US" title="Contact" accent="Us" lead="VPI Innovative Solutions - Location" image={factoryImage} /><PageIntro kicker="Contact Us" title={<>Get in <em>touch</em></>}><p className="page-lead">#13 P-A, “kSetra Tech Park”, KIADB 1st Main Road Phase-3, Koorgalli Industrial Area, Mysore, India - 570018</p><div className="contact-meta"><span>+91 9900911202</span><a href="mailto:vpisolutions@gmail.com">vpisolutions@gmail.com</a></div></PageIntro><section className="contact-section page-pad" data-theme="steel"><div className="contact-grid"><div className="contact-form-wrap"><QuoteForm /></div><div className="contact-map-card"><div className="contact-map-head"><span className="eyebrow cyan">OUR LOCATION</span><a href="https://www.google.com/maps/search/?api=1&query=Ksetra+Tech+Park+Koorgalli+Industrial+Area+Mysore+570018" target="_blank" rel="noreferrer">OPEN IN MAPS <ArrowUpRight size={14} /></a></div><iframe title="VPI Innovative Solutions map" src="https://www.google.com/maps?q=Ksetra+Tech+Park+Koorgalli+Industrial+Area+Mysore+570018&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div></div></section><VpiVisionFooter /></div>; }

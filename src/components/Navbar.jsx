...
const industryLinks = [["Automotive", "/industries/automotive"], ["Electronics", "/industries/electronics"], ["Robotics", "/industries/robotics"], ["Medical", "/industries/medical"], ["Aerospace", "/industries/aerospace"], ["Die & mould", "/industries/die-mould"], ["Energy", "/industries/energy"], ["Food", "/industries/food"], ["Advanced critical R&D", "/industries/critical-rd"], ["Telecom", "/industries/telecom"]];

function Dropdown({ label, links, menuKey, openMenu, setOpenMenu }) {
  const isOpen = openMenu === menuKey;
  return <div className="nav-dropdown-wrap"><button className="nav-link" data-testid={`${menuKey}-menu-button`} onClick={() => setOpenMenu(isOpen ? null : menuKey)}>{label} <ChevronDown size={13} /></button>{isOpen && <div className={`nav-dropdown ${menuKey === "industries" ? "industries-dropdown" : ""}`} data-testid={`${menuKey}-menu`}><span className="dropdown-kicker">{menuKey === "company" ? "VPI / COMPANY" : "SECTORS / 10"}</span>{links.map(([item, path]) => <NavLink key={path} to={path} data-testid={`${menuKey}-link-${item.toLowerCase().replaceAll(" ", "-")}`}>{item}</NavLink>)}</div>}</div>;
}
